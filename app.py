...
def tobs():
    ...
    query_start_date = dt.date(
        latest_date_obj.year, latest_date_obj.month, latest_date_obj.day) - dt.timedelta(days=366)
...
def start_only(start):
    ...
    date_range_max = session.query(Measurement.date).order_by(
    ...
    valid_entry = session.query(exists().where(
        Measurement.date == start)).scalar()
...
def start_end(start, end):
    ...
    # Check for valid entry of start date
    valid_entry_start = session.query(
        exists().where(Measurement.date == start)).scalar()

    # Check for valid entry of end date
    valid_entry_end = session.query(
        exists().where(Measurement.date == end)).scalar()
